 ...
 * @returns {{ name: string, owner: string, done: boolean, time: string}} Объект дела
 */
function makeTodoItemFromData(data) {
  ...
  if (!todoItem.time) todoItem.time = new Date();
 ...
 * @returns {{ name: string, owner: string, done: boolean, time: string}[]} Массив дел
 */
 ...
 * @param {{ name?: string, owner?: string, done?: boolean, time: string }} data - Объект с изменяемыми данными
 * @throws {TodoApiError} Дело с таким ID не найдено (statusCode 404)
 * @throws {TodoApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ name: string, owner: string, done?: boolean, time: string }} Объект дела
 */
